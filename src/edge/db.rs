...
impl Db {
    ...
    pub async fn create_tunnel(&self, name: &str) -> Result<TunnelRow> {
        ...
        sqlx::query(
            "INSERT INTO tunnels (id, name, token, created_at) VALUES (?, ?, ?, ?)",
        )
        .bind(&id)
        .bind(name)
        .bind(&token)
    ...
    pub async fn add_rule(
        ...
        sqlx::query(
            r#"INSERT INTO ingress_rules
               (id, tunnel_id, hostname, path_prefix, service_type, target, public_port, enabled)
               VALUES (?, ?, ?, ?, ?, ?, ?, 1)"#,
        )
        .bind(&id)
        .bind(tunnel_id)
    ...
    pub async fn rules_for_tunnel(&self, tunnel_id: &str) -> Result<Vec<IngressRule>> {
        let rows = self.list_rules(tunnel_id).await?;
